      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="flex flex-col items-center gap-2">
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(typeof window !== 'undefined' ? `${window.location.origin}/meetup/${vibeData.id}` : `https://app.example.com/meetup/${vibeData.id}`)}`}
              alt="QR Code"
              className="rounded border border-border"
            />
            <p className="text-xs text-muted-foreground text-center">Scan for activity page</p>
            <Link to={`/checkin?vibeId=${vibeData.id}&title=${encodeURIComponent(vibeData.title)}`}>

...
const HomePage = () => {
  ...
  const allStoriesWithSponsors = useMemo(() => {
    const list = allStories || [];
    if (user?.adFree || list.length === 0) return list;
    const mixed: any[] = [];
    const sponsorCount = Math.min(2, SPONSOR_STORIES.length);
    let sponsorIdx = 0;
    const interval = 4;
    list.forEach((s: any, i: number) => {
      mixed.push(s);
      if ((i + 1) % interval === 0 && sponsorIdx < sponsorCount) {
        mixed.push({ id: `sponsor-${SPONSOR_STORIES[sponsorIdx].id}`, type: 'sponsor', ...SPONSOR_STORIES[sponsorIdx] });
        sponsorIdx++;
      }
    });
    return mixed;
  }, [allStories, user?.adFree]);
  ...
  // Fetch communities (Skool-style on home)
            ...
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-52 rounded-xl">
                  ...
                  <DropdownMenuItem
                    onClick={() => {
                      if (isAuthenticated) navigate('/create-class');
                      else navigate('/login');
                    }}
                    className="flex items-center gap-3 py-3 cursor-pointer"
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() => {
                      if (isAuthenticated) navigate('/create-community');
                      else navigate('/login');
                    }}
                    className="flex items-center gap-3 py-3 cursor-pointer"
                  >
      ...
      <div className="px-4 pb-6 space-y-8">
        ...
        <section>
          <div className="flex items-center gap-4 overflow-x-auto hide-scrollbar pb-2">
            ...
            {allStoriesWithSponsors && allStoriesWithSponsors.length > 0 && allStoriesWithSponsors.slice(0, 18).map((story: any) => {
              if (story.type === 'sponsor') {
                return (
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground max-w-[60px] truncate">Sponsored</span>
                  </motion.div>
                );
              }
          ...
          <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2 -mx-4 px-4">
            {communitiesList.slice(0, 8).map((community: any) => (
              <motion.div
                ...
                className="flex-shrink-0 w-[140px] aspect-square rounded-2xl overflow-hidden bg-card border border-border hover:border-primary/30 transition-colors cursor-pointer active:scale-[0.98]"
                whileTap={{ scale: 0.98 }}
              >
                <div className="relative w-full h-full bg-muted">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-2">
                    <h3 className="font-semibold text-white text-sm truncate drop-shadow">{community.name}</h3>
                    <p className="text-xs text-white/90 flex items-center gap-1 mt-0.5">
                      <Users className="w-3 h-3 flex-shrink-0" />
                      {community.memberCount?.toLocaleString?.() ?? community.memberCount} members
                    </p>
                  </div>
                </div>
              </motion.div>
            ...
            <motion.button
              onClick={() => (isAuthenticated ? navigate('/create-community') : navigate('/login'))}
              className="flex-shrink-0 w-[140px] aspect-square rounded-2xl border-2 border-dashed border-border hover:border-primary/50 hover:bg-primary/5 transition-all flex flex-col items-center justify-center gap-2"
              whileTap={{ scale: 0.98 }}
            >
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                <Users2 className="w-5 h-5 text-primary" />
              </div>
              <span className="text-xs font-medium text-foreground text-center px-2">Create Community</span>
            </motion.button>
          </div>
        </section>

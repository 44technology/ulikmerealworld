...
import { getUserCommunityIds } from '@/hooks/useCommunities';
...
const ProfilePage = () => {
  ...
  const myCommunityCount = useMemo(() => getUserCommunityIds(user?.id || 'current-user').length, [user?.id]);

  const { data: myClassesData = [] } = useClasses(undefined, undefined, undefined, undefined, true);
  const myClassCount = Array.isArray(myClassesData) ? myClassesData.length : 0;
  ...
  return (
    <AppLayout>
      ...
      <div className="relative">
        ...
        {/* Profile info - avatar left on cover border, name/info left-aligned */}
        <div className="px-4 -mt-14 flex flex-row items-end gap-4">
          <div className="relative flex-shrink-0">
            <div className="ring-4 ring-background rounded-full overflow-hidden shadow-lg">
              <UserAvatar 
            ...
            <motion.button 
              ...
              className="absolute bottom-0 right-0 p-2 rounded-full bg-primary text-primary-foreground shadow-md border-2 border-background"
              whileTap={{ scale: 0.9 }}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 px-4">
          {stats.filter((s) => s.label !== 'Badges').map((stat) => {
            const Icon = stat.icon;
            return (
              <motion.button
                key={stat.label}
                className="card-elevated p-4 rounded-2xl text-center border border-border/50 hover:border-primary/20 transition-colors"
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.98 }}
        ...
        {/* Badges - elegant section */}
        <div className="mt-6 px-4">
          <motion.button
            onClick={() => navigate('/badges')}
            className="w-full text-left rounded-2xl overflow-hidden border border-border/50 bg-gradient-to-br from-card to-card/80 hover:from-primary/5 hover:to-primary/10 hover:border-primary/20 transition-all duration-200 shadow-sm hover:shadow-md"
            whileTap={{ scale: 0.99 }}
          >
            <div className="p-4 flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500/20 to-orange-500/10 flex items-center justify-center flex-shrink-0 border border-amber-500/20">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground">
                  {statsLoading ? '…' : `${statsData?.badges ?? 0} earned`} · Tap to view all
                </p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-2xl font-bold text-foreground">{statsLoading ? '–' : statsData?.badges ?? 0}</span>
          ...
          {/* Quick links (formerly under More) */}
          <div className="mt-6 px-4">
            ...
            {[
              ...
              return (
                <motion.button
                  ...
                  className="flex items-center gap-3 w-full p-4 hover:bg-muted/50 active:bg-muted text-left border-b border-border last:border-b-0"
                  whileTap={{ scale: 0.99 }}
                >
          ...
          {/* Quick links */}
          <div className="mt-6 px-4 space-y-2 pb-4">
            {[

...
const ClassesPage = () => {
  ...
  // Fetch mentors (hosts who offer mentorship classes)
  ...
  const backendClassesFormatted = backendClasses && backendClasses.length > 0
    ? backendClasses.map(c => {
        ...
        return {
          ...
          host: {
            name: (c as any).instructor?.name || (c as any).creator?.displayName || c.venue?.name || 'Host',
            avatar: (c as any).instructor?.avatar || (c as any).creator?.avatar || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150',
    ...
    : classes.map(c => ({
        ...c,
        host: { name: c.instructor.name, avatar: c.instructor.avatar, rating: c.instructor.rating },
          ...
          >
            <div className="relative h-36">
              ...
              <div className="absolute top-2 right-2 flex items-center gap-2">
                ...
                <div className="px-2 py-1 rounded-full bg-card/90 backdrop-blur-sm flex items-center gap-1">
                  ...
                  <span className="text-xs font-medium">{((classItem as any).host?.rating ?? (classItem as any).instructor?.rating) || 4.5}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <UserAvatar src={(classItem as any).host?.avatar ?? (classItem as any).instructor?.avatar} alt={(classItem as any).host?.name ?? (classItem as any).instructor?.name} size="sm" />
                <span className="text-sm text-muted-foreground">{(classItem as any).host?.name ?? (classItem as any).instructor?.name}</span>

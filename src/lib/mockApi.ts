...
const CLASS_ENROLLMENTS_STORAGE_KEY = 'mock_class_enrollments';
...
const initDummyData = () => {
  ...
  if (classes.length === 0) {
    ...
    // Update existing classes: set price 50 only if missing; keep 0 as free; ensure one free class (Salsa)
    classes = classes.map((c: any) => ({
      ...c,
      price: c.price === undefined || c.price === null ? 50 : c.price,
    }));
    const salsaClass = classes.find((c: any) => c.id === 'class-3');
    if (salsaClass) {
      salsaClass.price = 0;
    }
...
export const mockApiRequest = async <T = any>(
  ...
  if (pathname.includes('/meetups')) {
    ...
    if (method === 'GET') {
      ...
      if (searchParams.get('communityId')) {
        const communityId = searchParams.get('communityId')!;
        filtered = filtered.filter((m: any) => m.communityId === communityId);
      }
      return {
        success: true,
        data: filtered,
  ...
  if (pathname.includes('/posts')) {
    ...
    if (method === 'GET') {
      ...
      const communityId = searchParams.get('communityId');
      if (communityId) {
        list = list.filter((p: any) => p.communityId === communityId);
      }
      return {
        success: true,
        data: list,
  ...
  // Default syllabus for classes (so lesson progress can be shown)
  const getDefaultSyllabusForClass = (cls: any) => {
    if (cls.syllabus && Array.isArray(cls.syllabus) && cls.syllabus.length > 0) return cls;
    const skill = cls.skill || 'Class';
    const baseId = cls.id || 'class';
    return {
      ...cls,
      syllabus: [
        {
          ...
          lessons: [
            { id: `${baseId}-lesson-1`, title: 'Overview', duration: '30 min', description: 'Introduction' },
            { id: `${baseId}-lesson-2`, title: 'Core Concepts', duration: '45 min', description: 'Key concepts' },
          ],
        },
        {
          id: `${baseId}-mod-2`,
          ...
          lessons: [
            { id: `${baseId}-lesson-3`, title: 'Step-by-Step', duration: '40 min', description: 'Guided practice' },
            { id: `${baseId}-lesson-4`, title: 'Projects', duration: '50 min', description: 'Apply what you learned' },
          ],
        },
        {
          id: `${baseId}-mod-3`,
          ...
          lessons: [
            { id: `${baseId}-lesson-5`, title: 'Advanced Tips', duration: '35 min', description: 'Take it further' },
          ],
        },
      ],
    };
  ...
  if (pathname.includes('/classes')) {
    ...
    if (method === 'GET') {
      ...
      if (pathname.includes('/my-classes')) {
        const enrollments = getStorage(CLASS_ENROLLMENTS_STORAGE_KEY, []);
        const myEnrollments = enrollments.filter((e: any) => e.userId === currentUserId);
        const withClass = myEnrollments.map((e: any) => {
          const cls = classes.find((c: any) => c.id === e.classId) || e.class;
          const enriched = cls ? getDefaultSyllabusForClass({ ...cls }) : cls;
          return { ...e, class: enriched };
        });
        return { success: true, data: withClass } as T;
      }

      if (classId && classId !== 'classes' && classId !== 'nearby') {
      ...
      const communityId = searchParams.get('communityId');
      if (communityId) {
        list = list.filter((c: any) => c.communityId === communityId);
      }
      return {
        success: true,
        data: list,
    ...
    if (pathname.includes('/enroll')) {
      ...
      const enrichedClass = getDefaultSyllabusForClass({ ...cls });
      const enrollment = {
        id: enrollmentId,
        classId: cls.id,
        userId: currentUserId,
      ...
      setStorage(CLASS_ENROLLMENTS_STORAGE_KEY, enrollments);
  ...
  if (pathname.includes('/matches')) {
    ...
    if (method === 'POST') {
      ...
      const newMatch = {
        ...
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        user: receiverId ? { id: receiverId, firstName: '', lastName: '', displayName: 'User' } : undefined,
        isSender: true,
      };
...
export const mockApiUpload = async <T = any>(
  ...
  if (url.includes('/posts')) {
    ...
    const newPost = {
      ...
      creator: { id: user.id, firstName: user.firstName, lastName: user.lastName, displayName: user.displayName, avatar: user.avatar },
      likes: 0,
      comments: [],
      commentsCount: 0,
      createdAt: new Date().toISOString(),
    ...
    posts.push(newPost);
    setStorage(POST_STORAGE_KEY, posts);
    return { success: true, data: newPost } as T;
  }

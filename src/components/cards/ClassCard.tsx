      ...
      {image && (
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute top-3 right-3 flex items-center gap-1.5">
            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
              (price === undefined || price === null || price === 0) ? 'bg-green-500/90 text-white' : 'bg-card/90 backdrop-blur-sm text-foreground'
            }`}>
              {(price === undefined || price === null || price === 0) ? 'Free' : `$${price}`}
            </span>
          </div>
        </div>
      )}
        ...
        {!image && (
          <span className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${
            (price === undefined || price === null || price === 0) ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-primary/10 text-primary'
          }`}>
            {(price === undefined || price === null || price === 0) ? 'Free' : `$${price}`}
          </span>
        )}
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
              {(price === undefined || price === null || price === 0) ? (
                ...
                <><DollarSign className="w-4 h-4" /><span>${price}</span></>
              )}
            </div>
        </div>
